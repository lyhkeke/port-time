...
public class UserDAOImpl implements UserDAO{
	...
	public int addUser(User user) {
		String sql = "INSERT INTO `ttt`.`tab_user`(`sid`, `username`, `password`, `gender`, `phone`, `email`, `totalscore`,`appraisenum`, `permission`) VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public int deleteById(int uid) {
		String sql="DELETE FROM `ttt`.`tab_user` WHERE `uid` =?";
...
//		//String sql ="UPDATE `ttt`.`user` SET sid=?,gender=?,phone=?,email=? WHERE `uid` = 4"
//		return 0;
//	}

	public int updatePwd(Integer uid,String password) {
		String sql="UPDATE `ttt`.`tab_user` SET `password` = ? WHERE `uid` = ?";	
		return dbUtil.update(sql, password,uid);
	}

	@Override
	public int updateUser(User user) {
		String sql="UPDATE `ttt`.`tab_user` SET `sid` = ?, `username` = ?, `password` = ?, `gender` = ?, `phone` = ?, `email` = ?, `totalscore` = ?, `appraisenum` = ?, `permission` = ? WHERE `uid` = ? ";
