...
public class PositionDAOImpl implements PositionDAO {
	...
	public int deletePositionById(int pid) {
		String sql="DELETE FROM `ttt`.`tab_position` WHERE `pid` =?";
	...
	public int updatePosition(Position position) {
		String sql="UPDATE `ttt`.`tab_position` SET `pname` = ? WHERE `pid` = ?";
		return dbUtil.update(sql, position.getPname(),position.getPid());
	}

	@Override
	public int addPosition(Position position) {
		String sql = "INSERT INTO `ttt`.`tab_position`(`pname`) VALUES (?)";
