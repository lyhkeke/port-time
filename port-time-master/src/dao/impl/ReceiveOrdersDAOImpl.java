...
public class ReceiveOrdersDAOImpl implements ReceiveOrdersDAO {
	...
	public int save(ReceiveOrders receiveOrders) {
		String sql ="INSERT INTO `ttt`.`tab_receiveorders`(`oid`, `uid`, `status`,receivetime) VALUES (?,?, ?, ?)";
	...
	public int deleteById(Integer id) {
		String sql="DELETE FROM `ttt`.`tab_receiveorders` WHERE `id` = ?";
	...
	public int update(ReceiveOrders receiveOrders) {
		String sql="UPDATE `ttt`.`tab_receiveorders` SET `oid` = ?, `uid` = ?, `status` = ? , where receivetime= ? WHERE `id` = ?";
	...
	public List<ReceiveOrders> findAll() {
		String sql ="SELECT * FROM `ttt`.`tab_receiveorders` by uid desc";
		ResultSet rs = dbUtil.query(sql);
	...
	public ReceiveOrders findById(Integer id) {
		String sql ="SELECT * FROM `ttt`.`tab_receiveorders`  where id=? ";
	...
	public List<ReceiveOrders> findByUid(Integer uid) {
		String sql ="SELECT * FROM `ttt`.`tab_receiveorders` where uid =? order by uid desc";
	...
	public List<ReceiveOrders> findByOid(Integer oid) {
		String sql ="SELECT * FROM `ttt`.`tab_receiveorders` where oid =? order by receivetime desc";
	...
	public int updateByStatus(Integer status,Integer id) {
		String sql ="UPDATE `ttt`.`tab_receiveorders` SET `status` = ? WHERE `id` = ?";
