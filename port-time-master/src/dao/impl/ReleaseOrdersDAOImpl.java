...
public class ReleaseOrdersDAOImpl implements ReleaseOrdersDAO {
	...
	public int save(ReleaseOrders releaseOrders) {
		String sql="INSERT INTO `ttt`.`tab_releaseorders`(`sid`, `pid`, `title`, `description`, `paymethod`, `recruitnum`, `releasetime`, `uid`, `salary`, `status`) VALUES (?,?,?,?,?,?,?,?,?,?)";
	...
	public int update(ReleaseOrders releaseOrders) {
		String sql = "UPDATE `ttt`.`tab_releaseorders` SET `sid` = ?, `pid` = ?, `title` = ?, `description` = ?, `paymethod` = ?, `recruitnum` = ?, `releasetime` = ?, `uid` = ?, `salary` = ?, `status` = ? WHERE `oid` = ?";
	...
	public int deleteById(Integer id) {
		String sql="DELETE FROM `ttt`.`tab_releaseorders` WHERE `oid` = ?";
	...
	public List<ReleaseOrders> findThree(Integer uid) {
		String sql="SELECT * FROM `ttt`.`tab_releaseorders` where uid = ?   ORDER BY releasetime desc LIMIT 0, 3";
		List<ReleaseOrders> list=new ArrayList<ReleaseOrders>();
		ResultSet rs = dbUtil.query(sql, uid);
		handOrdersList(rs, list);
		return list;
	...
	public List<ReleaseOrders> findByUid(Integer uid) {
		String sql="SELECT * FROM `ttt`.`tab_releaseorders` where uid = ?   ORDER BY releasetime desc ";
		List<ReleaseOrders> list=new ArrayList<ReleaseOrders>();
		ResultSet rs = dbUtil.query(sql, uid);
		handOrdersList(rs, list);
		return list;
	...
	public int deleteByUid(Integer uid) {
		String sql="DELETE FROM `ttt`.`tab_releaseorders` WHERE `uid` = ?";
